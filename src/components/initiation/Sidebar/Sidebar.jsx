...
const Sidebar = ({ ...props }) => {
  ...
  const { classes, logo, routes, color , selectedDashboardItems , isDashboardContent } = props;
  ...
  const isActiveRoute = (routeName, origin) => { 
    if(isDashboardContent){
        const reduxDashbordUrl = selectedDashboardItems ? selectedDashboardItems[0]?.myDashboardName : '';
        return( routeName ===  '/' + reduxDashbordUrl);
    }
   
    const pathMatched = props.location.pathname.indexOf(routeName) > -1 ? true : false;
    const originMatched = window.location.origin.indexOf(origin) > -1 ? true : true; //TODO should be changed once origin is available
  ...
  const selectRoute = (route) => {
    dispatch(setDashbordNavigationStatus(true));
    ...
    //     route.origin + route.layout + route.path + '?' + qs.stringify({
