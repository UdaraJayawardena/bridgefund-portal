...
function CustomInputBox(props) {
  ...
  const {
    ...
    helperText, placeholder, readOnly, disabled, defaultValue, onAdd, onRemove, subType, changeToFormatType,
    fieldName, multiline, rows, rowsMax, required, showError, isNoneInDropDownList } = props;

  const [error, setError] = React.useState(showError);
  ...
  if (type === 'text') return (
    <FormControl size="small" variant="outlined" className={formControlStyle} fullWidth error={error}>
      <TextField
        id={id}
        label={label}
        variant="outlined"
    ...
    <FormControl size="small" variant="outlined" className={formControlStyle} fullWidth>
      <TextField
        value={value}
        onChange={(e) => handleChange(name, e.target.value)}
        name={name}
    ...
    <FormControl size="small" variant="outlined" className={classes.formControl} fullWidth error={error}>
...
CustomInputBox.propTypes = {
  ...
  changeToFormatType: PropTypes.oneOf(['CamelCase']),
