...
import {currencyConverter, toFixed} from "lib/initiation/utility";
...
class SplitDirectDebit extends Component {
  ...
  setDefaultData = () => {
    ...
    if (!this.props.isChangeContract) {
      splitDdPeriod["1"].noOfDD = this.props.plannedNoOfDD;
      splitDdPeriod["1"].amount = toFixed(this.props.totalLoanAmount / this.props.plannedNoOfDD);
  ...
  handleChange = (name, value, period) => {
    ...
    if (period) {
      ...
      if (Math.abs(splitDdPeriod.total - this.props.totalLoanAmount) > (0.01 * this.props.plannedNoOfDD)) {
        this.props.displayNotification(`Difference of total loan amount & Sum of split dd amounts should not exceed ${EURO(0.01 * this.props.plannedNoOfDD)}`, 'warning');
        return;
      }
    }
  ...
  render() {
    const { classes, splitDdPeriod } = this.props;

    return (
      <Table>
        <TableHead>
          <TableRow>
            <TableCell className={clx(classes.tableHeadCell, classes.bold)}>#DD</TableCell>
            <TableCell className={clx(classes.tableHeadCell, classes.bold, classes.alignCenter)}>DD-Amount</TableCell>
            <TableCell className={clx(classes.tableHeadCell, classes.bold, classes.alignCenter)}>Total</TableCell>
            <TableCell className={clx(classes.tableHeadCell, classes.bold, classes.alignCenter)}>Start Date</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          <TableRow>
            ...
            <TableCell className={clx(classes.tableCell, classes.alignRight)}>{EURO(splitDdPeriod['1'].total)}</TableCell>
          ...
          <TableRow>
            ...
            <TableCell className={clx(classes.tableCell, classes.alignRight)}>{EURO(splitDdPeriod['2'].total)}</TableCell>
          </TableRow>
          <TableRow>
            <TableCell className={clx(classes.tableCell)}>
              <TextField
            ...
            <TableCell className={clx(classes.tableCell, classes.alignRight)}>{EURO(splitDdPeriod['3'].total)}</TableCell>
          </TableRow>
          <TableRow>
            <TableCell className={clx(classes.tableCell)}>&nbsp;</TableCell>
            <TableCell className={clx(classes.tableCell)}>&nbsp;</TableCell>
            <TableCell className={clx(classes.tableCell, classes.bold, classes.alignRight)}>{EURO(splitDdPeriod.total)}</TableCell>
          </TableRow>
        </TableBody>
      </Table>
    );
...
SplitDirectDebit.propTypes = {
  ...
  isChangeContract: PropTypes.bool.isRequired,
