...
export const restoreWorkflows = () => async dispatch => {

  const request = {
    url: INITIATION_GATEWAY_URL('/restore-workflows')
  };

  try {
    const response = await httpService.post(request, dispatch);

    dispatch(displayNotification('Workflows successfully restored', 'success'));
    dispatch(handleSnapshotDialogLoading(false));
    ...
    dispatch(displayNotification('Workflows restore failed', 'success'));
    dispatch(handleSnapshotDialogLoading(false));
  }
};
